...
Original file is located at
    https://colab.research.google.com/drive/1tf3YC9qHK7Gg_eIQ-sIckOw1X8oi9eCD

# EMG Gesture Classification: Comparative Analysis of Deep Learning Architectures

   This notebook implements and compares three deep learning approaches for classifying hand gestures from electromyography (EMG) signals.
   
   The EMG data was preprocessed using the pipeline defined in `1_process_data.ipynb`, which handles data cleaning, formatting, and validation.
   
   Deep Learning Architectures used:
   1. **1D Convolutional Neural Network (CNN)**: Effective at extracting spatial features from the EMG channels <br> https://ieeexplore.ieee.org/document/7514991/
   
   2. **Long Short-Term Memory (LSTM)**: Specialized in capturing temporal dependencies in the signal sequence <br> https://ieeexplore.ieee.org/document/10017115

   3. **Hybrid CNN-LSTM Model**: Combines spatial feature extraction with temporal modeling <br> https://ieeexplore.ieee.org/document/8973549
"""
...
plot_emg_channels(X_tensor.cpu(), title="Sample Window", sample_idx=0)
...
"""   ## Model Architectures
   ...
   ### 1D CNN Model
   A convolutional neural network designed to capture spatial patterns in the EMG signals across channels. Features:
...
print(CNN1D(num_channels=8, num_classes=7).to(device))
...
print(LSTMModel(num_channels=8, num_classes=7).to(device))
...
# Print model summary
print(CNNLSTMModel( num_channels=8, num_classes=7).to(device))

"""## Hyperparameters and training

   The three models were trained for 100 epochs with a batch size of 32. The comparison of their performance reveals:

   - **CNN Model**: Shows good spatial feature extraction with accuracy of {test_acc_cnn:.4f}
   - **LSTM Model**: Captures temporal dependencies with accuracy of {test_acc_lstm:.4f}
   - **Hybrid CNN-LSTM Model**: Leverages both spatial and temporal information with accuracy of {test_acc_cnn_lstm:.4f}

   The hybrid model generally shows the most robust performance, likely due to its ability to extract both types of features from the EMG signals.
"""
...
cnn_model = CNN1D(num_channels=x_train_norm.shape[2], num_classes=y_train_oh.shape[1])
...
lstm_model = LSTMModel(num_channels=x_train_norm.shape[2], num_classes=y_train_oh.shape[1])
...
cnn_lstm_model = CNNLSTMModel( num_channels=x_train_norm.shape[2], num_classes=y_train_oh.shape[1])
...
def plot_comparison(history_1dcnn, history_lstm, history_hybrid=None):
    ...
    Organizes plots with training metrics in the first row and validation metrics in the second row.
    ...
    save_path = 'results/2_emg_model_comparison.png'
    ...
    fig.suptitle("EMG Dataset - Training and Validation Results", fontsize=20, fontweight='bold')

    # Colors for different models
    colors = {'1D CNN': 'blue', 'LSTM': 'red', 'CNN-LSTM Hybrid': 'green'}

    # ---------------- TRAINING ACCURACY (Top Left) ----------------
    axes[0, 0].plot(epochs, history_1dcnn['train_acc'], '-', color=colors['1D CNN'], label='1D CNN')
    axes[0, 0].plot(epochs, history_lstm['train_acc'], '-', color=colors['LSTM'], label='LSTM')
    if history_hybrid:
        axes[0, 0].plot(epochs, history_hybrid['train_acc'], '-', color=colors['CNN-LSTM Hybrid'], label='CNN-LSTM Hybrid')
    ...
    axes[0, 1].plot(epochs, history_1dcnn['train_loss'], '-', color=colors['1D CNN'], label='1D CNN')
    axes[0, 1].plot(epochs, history_lstm['train_loss'], '-', color=colors['LSTM'], label='LSTM')
    if history_hybrid:
        axes[0, 1].plot(epochs, history_hybrid['train_loss'], '-', color=colors['CNN-LSTM Hybrid'], label='CNN-LSTM Hybrid')
    ...
    axes[1, 0].plot(epochs, history_1dcnn['val_acc'], '--', color=colors['1D CNN'], label='1D CNN')
    axes[1, 0].plot(epochs, history_lstm['val_acc'], '--', color=colors['LSTM'], label='LSTM')
    if history_hybrid:
        axes[1, 0].plot(epochs, history_hybrid['val_acc'], '--', color=colors['CNN-LSTM Hybrid'], label='CNN-LSTM Hybrid')
    ...
    # ---------------- VALIDATION LOSS (Bottom Right) ----------------
    axes[1, 1].plot(epochs, history_1dcnn['val_loss'], '--', color=colors['1D CNN'], label='1D CNN')
    axes[1, 1].plot(epochs, history_lstm['val_loss'], '--', color=colors['LSTM'], label='LSTM')
    if history_hybrid:
        axes[1, 1].plot(epochs, history_hybrid['val_loss'], '--', color=colors['CNN-LSTM Hybrid'], label='CNN-LSTM Hybrid')
...
The results from the deep learning experiments on EMG gesture classification demonstrate the effectiveness of the three models tested. Among the architectures, the hybrid CNN-LSTM model outperformed the other models, achieving the highest test accuracy of 96.24% and a test loss of 0.1903. This model also displayed high precision and recall across all gesture classes, with exceptional performance on gesture class 6, reaching a perfect recall of 100%. However, gesture class 3 exhibited slightly lower performance, with a recall of 86%.

The CNN and LSTM models also yielded promising results but did not match the hybrid model's performance. Visualization of the training and validation metrics showed that the hybrid CNN-LSTM model maintained consistent improvement throughout training, indicating effective learning of both spatial and temporal features.

Confusion matrices further confirmed the models' ability to differentiate between gesture classes, with the hybrid model providing the clearest separation. These findings suggest that combining spatial and temporal feature extraction leads to superior performance for EMG-based gesture classification.
"""